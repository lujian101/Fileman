...
namespace lua_tinker
{
	...
	template<typename RVal, typename T1, typename T2>
	void push_functor(lua_State *L, RVal (*)(T1,T2))
	{
	...
	struct table_obj
	{
		...
		template<typename T>
		void array_set(int index, T object)
		{
			if(validate())
			{
	...
	struct table
	{
		...
		T array_get( Idx index )
		{
			return m_obj->array_get<T>( (int)index );
		}
		template<typename T, typename Idx>
		void array_set( Idx index, T object )
		{
			return m_obj->array_set<T>( (int)index, object );
		}
		table_obj*		m_obj;
	};
